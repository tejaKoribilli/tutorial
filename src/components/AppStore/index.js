...
class AppStore extends Component {
  ...
  getFetchAppsList = () => {
    const {appsList, activeTab} = this.state
    const filteredData = appsList.filter(each =>
      each.category.includes(activeTab),
    )
  ...
  render() {
    const {appsList, activeTab, searchInput} = this.state
    const fetchAppsList = this.getFetchAppsList()

    const searchResult = fetchAppsList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    return (
      <div className="bg-container">
        <h1 className="heading-apStore">App Store</h1>
